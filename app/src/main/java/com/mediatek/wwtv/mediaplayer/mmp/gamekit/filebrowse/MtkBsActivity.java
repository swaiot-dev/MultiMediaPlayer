...
//import com.mediatek.wwtv.mediaplayer.mmp.gamekit.filebrowse.MFlimBsActivity.BrowseStatus;
...
public class MtkBsActivity extends Activity  implements
Observer {
	...
	protected MmpConst.BrowseStatus mFromStatus = MmpConst.BrowseStatus.NORMAL_STATUS;
	protected MmpConst.BrowseStatus mBackStatus = MmpConst.BrowseStatus.NORMAL_STATUS;
...
//	private Thread mRecursThread;
	...
	private void refreshBoardView(List<FileAdapter> fileList) {
		...
		if (mFromStatus == MmpConst.BrowseStatus.FROM_PLAY_STATUS &&
			mFromType != MutilMediaConst.CONTENT_UNKNOW ){
			reloadContent();
			mFromType = MutilMediaConst.CONTENT_UNKNOW;
			mFromStatus = MmpConst.BrowseStatus.NORMAL_STATUS;
			return;
			//from exra list mode or grid mode
		} else if (mFromStatus == MmpConst.BrowseStatus.FROM_GRID_STATUS ||
				mFromStatus == MmpConst.BrowseStatus.FROM_LIST_STATUS){
			reloadView(mFromPostion);
			mBackStatus = mFromStatus;
			mFromStatus = MmpConst.BrowseStatus.NORMAL_STATUS;
			return;
			//normal enter folder or leave folder
		} else if (mFromStatus == MmpConst.BrowseStatus.NORMAL_STATUS) {
			if (idx >= 0) {
				reloadView(idx);
				return;
			} else {
