...
public class Imageshowimpl implements IImageshow {
   ...
   * imagepL.isBegin(Const.FILTER_IMAGE))){ isEnd = true; } MmpTool.LOG_INFO("isEnd : " + isEnd);
   * return isEnd; }
   */
  ...
  private Bitmap rightRotate(Bitmap bitmap, int dg) {
    ...
    if (dg < Const.ORIENTATION_ARRAY.length){
  ...
  public PhotoUtil transfBitmap(String transfPath) {
    ...
    if (i > 1) {
      ...
      try {
        trBmp.setBitmap(BitmapFactory.decodeFile(transfPath, bmpFactoryOptions));
  ...
  private boolean isValidSizePhoto(long size) {
    if (LocOrNet != ConstPhoto.LOCAL) {
      if (size >= 0 && size < FileConst.MAX_PHOTO_SIZE_NET) {
        return true;
      }
      return false;
    }
