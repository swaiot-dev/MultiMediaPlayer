...
import com.mediatek.wwtv.mediaplayer.mmpcm.threedimen.photo.IPlayback;
...
public class MPlayback implements IPlayback{
	...
	private IThrdListener mEventListener;
	...
	public void decode3DPhoto(String path,int source){
		...
		if( path == null){
			Util.printAutoTestImage3D(path,"init fail");
		...
		if(mPhotoPlayer != null ){
			...
			if(!dumy){
				...
				try{
					...
					if(!cancelState){
						synchronized(this) {
							if(!cancelState){
								...
								Util.printAutoTestImage3D(path,"Decode success");
							}
						}
					}
					if(!cancelState){
						synchronized(this) {
							if(null != mtkPhotoHandler && !cancelState){
								...
								Util.printAutoTestImage3D(path,"play done pass");
					...
					Util.printAutoTestImage3D(path,"exception fail");
	...
	*/
	 ...
	 * @param listener
	 */
	public void setEventListener(IThrdListener listener){
