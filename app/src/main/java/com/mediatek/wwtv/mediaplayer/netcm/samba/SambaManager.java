...
public class SambaManager {
  ...
  private static final String mSecondRoot = "smb://GCN/";
  ...
  private final LinkedList<SmbFile> mListSmbFile = new LinkedList<SmbFile>();
  private final Hashtable<Integer, SmbFileFilter> mFilter = new Hashtable<Integer, SmbFileFilter>();
  ...
  static class MultiMediaFileFilter implements SmbFileFilter {
    public boolean accept(SmbFile arg0) throws SmbException {
      ...
      if (localLOGV)
        NetLog.d(TAG, "in MultiMediaFileFilter");
      String filename = arg0.getName();
      if (filename.endsWith("/")) {
        filename = filename.substring(0, filename.lastIndexOf('/'));// before
                                                                    // action:filename.substring(0,
                                                                    // filename.lastIndexOf('/'));
      }
      int type = arg0.getType();
      if (localLOGV)
        NetLog.d(TAG, "in MultiMediaFileFilter filename = " + filename + "type = " + type);
      ...
      if (arg0.isDirectory() || isNeedFile(filename, FileConst.audioSuffix)
        ...
        if (localLOGV)
          NetLog.d(TAG, "in MultiMediaFileFilter return true");
        return true;
      }
      return false;
    }
  ...
  static class VideoFileFilter implements SmbFileFilter {
    public boolean accept(SmbFile arg0) throws SmbException {
      // TODO Auto-generated method stub
      if (localLOGV)
        NetLog.d(TAG, "in VideoFileFilter");
      String filename = arg0.getName();
      if (filename.endsWith("/")) {
        filename = filename.substring(0, filename.lastIndexOf('/'));// before
                                                                    // action:filename.substring(0,
      ...
      int type = arg0.getType();
      if (localLOGV)
        NetLog.d(TAG, "in VideoFileFilter filename = " + filename + "type = " + type);
      if (type == SmbFile.TYPE_PRINTER || type == SmbFile.TYPE_NAMED_PIPE) {
        return false;
      }
      filename = filename.toLowerCase();
      if (arg0.isDirectory() || isNeedFile(filename, FileConst.videoSuffix)) {
        if (localLOGV)
          NetLog.d(TAG, "in VideoFileFilter return true");
        return true;
      }
      return false;
    }
  ...
  static class AudioFileFilter implements SmbFileFilter {
    public boolean accept(SmbFile arg0) throws SmbException {
      // TODO Auto-generated method stub
      if (localLOGV)
        NetLog.d(TAG, "in AudioFileFilter");
      String filename = arg0.getName();
      if (filename.endsWith("/")) {
        filename = filename.substring(0, filename.lastIndexOf('/'));// before
                                                                    // action:filename.substring(0,
      ...
      int type = arg0.getType();
      if (localLOGV)
        NetLog.d(TAG, "in AudioFileFilter filename = " + filename + "type = " + type);
      if (type == SmbFile.TYPE_PRINTER || type == SmbFile.TYPE_NAMED_PIPE) {
        return false;
      }
      filename = filename.toLowerCase();
      if (arg0.isDirectory() || isNeedFile(filename, FileConst.audioSuffix)) {
        if (localLOGV)
          NetLog.d(TAG, "in AudioFileFilter return true");
        return true;
      }
      return false;
    }
  ...
  static class ImageFileFilter implements SmbFileFilter {
    public boolean accept(SmbFile arg0) throws SmbException {
      // TODO Auto-generated method stub
      if (localLOGV)
        NetLog.d(TAG, "in ImageFileFilter");
      String filename = arg0.getName();
      if (filename.endsWith("/")) {
        filename = filename.substring(0, filename.lastIndexOf('/'));// before
                                                                    // action:filename.substring(0,
      ...
      int type = arg0.getType();
      if (localLOGV)
        NetLog.d(TAG, "in ImageFileFilter filename = " + filename + "type = " + type);
      if (type == SmbFile.TYPE_PRINTER || type == SmbFile.TYPE_NAMED_PIPE) {
        return false;
      }
      filename = filename.toLowerCase();
      if (arg0.isDirectory() || isNeedFile(filename, FileConst.photoSuffix)) {
        if (localLOGV)
          NetLog.d(TAG, "in ImageFileFilter return true");
        return true;
      }
      return false;
    }
  ...
  */
  static class ThrdPhotoFileFilter implements SmbFileFilter {
    public boolean accept(SmbFile arg0) throws SmbException {
      ...
      int type = arg0.getType();
      if (localLOGV)
        NetLog.d(TAG, "in ThrdPhotoFileFilter filename = " + filename + "type = " + type);
      if (type == SmbFile.TYPE_PRINTER || type == SmbFile.TYPE_NAMED_PIPE) {
        return false;
      }
      filename = filename.toLowerCase();
  ...
  static class TextFileFilter implements SmbFileFilter {
    public boolean accept(SmbFile arg0) throws SmbException {
      // TODO Auto-generated method stub
      if (localLOGV)
        NetLog.d(TAG, "in TextFileFilter");
      String filename = arg0.getName();
      if (filename.endsWith("/")) {
        filename = filename.substring(0, filename.lastIndexOf('/'));// before
                                                                    // action:filename.substring(0,
      ...
      int type = arg0.getType();
      if (localLOGV)
        NetLog.d(TAG, "in TextFileFilter filename = " + filename + "type = " + type);
      if (type == SmbFile.TYPE_PRINTER || type == SmbFile.TYPE_NAMED_PIPE) {
        return false;
      }
      filename = filename.toLowerCase();
      if (arg0.isDirectory() || isNeedFile(filename, FileConst.textSuffix)) {
        if (localLOGV)
          NetLog.d(TAG, "in TextFileFilter return true");
        return true;
      }
      return false;
    }
  ...
  public void initSamba() {
    ...
    jcifs.Config.setProperty("jcifs.netbios.retryTimeout", "10000");
    jcifs.Config.setProperty("jcifs.netbios.cachePolicy", "24000");

//    jcifs.Config.setProperty("jcifs.netbios.baddr", "172.26.174.255");
  ...
  public static void login(String path, String username, String password)
    ...
    if (localLOGV)
      NetLog.d(TAG, "[login3][path]: " + path + ", [name]: " + username + ", [pwd]: "
          + password);
    if (checkFirstLevel(path)) {
      if (localLOGV)
        NetLog.d(TAG, "[login3]: FirstLevel");
      SmbFile smbFile = new SmbFile(path);
      if (localLOGV)
        NetLog.d(TAG, "[login3]: smbFile.getType():" + smbFile.getType());
      if (smbFile.getType() == SmbFile.TYPE_WORKGROUP) {
        String domainName = getDomainName(path);
        if (domainName != null) {
          NetLog.d(TAG, "[login3]: FirstLevel, Domain login!");
        ...
        {
          if (localLOGV)
            NetLog.d(TAG, "[login3]: FirstLevel, Domain name NULL!");
        }
      }
    }
    if (localLOGV)
      NetLog.d(TAG, "[login3]: Non-FirstLevel!!! put path to table(user and pwd)");
    /**
  ...
  public static void login(String path, String domain, String username,
      ...
      SmbAuthException {
    if (localLOGV)
      NetLog.d(TAG, "[login4][path]: " + path + ", [domain]: " + domain + ", [name]: "
          + username + ", [pwd]: " + password);
    if ((domain == null) && (username == null) && (password == null)) {
      if (localLOGV)
        NetLog.d(TAG, "[login4]: All 3 filed are NULL");
     ...
     */
    if (localLOGV)
      NetLog.d(TAG, "[login4][domain]: " + domain + " [name]: " + username + " [pwd]: "
          + password);
    if (username == null) {
      username = "";
    }
  ...
  public LinkedList<String> getSmbFileList(String path)
    ...
    if (localLOGV)
      NetLog.d(TAG, "[getSmbFileList1][path]: " + path);
    ...
    if (localLOGV)
      NetLog.d(TAG, "[getSmbFileList1]start list files: " + System.currentTimeMillis());
    SmbFile[] fList = smbFile.listFiles();
    if (localLOGV)
      NetLog.d(TAG, "[getSmbFileList1]end list files: " + System.currentTimeMillis());
  ...
  public LinkedList<String> getSmbFileList(String path, int filterType)
    ...
    if (localLOGV)
      NetLog.d(TAG, "[getSmbFileList2][path]: " + path);
    SmbFile smbFile = getSmbFileIns(path);
    switch (smbFile.getType()) {
      case SmbFile.TYPE_FILESYSTEM:
        NetLog.d(TAG, path + " :[TYPE_FILESYSTEM]");
    ...
    if (localLOGV) {
      NetLog.d(TAG, "[getSmbFileList2]start list files startTime: " + startTime);
    }
    SmbFile[] fList = smbFile.listFiles(mFilter
        .get(new Integer(filterType)));
    NetLog.d(TAG, "[getSmbFileList2]end list files: " + (System.currentTimeMillis() - startTime));
    for (int i = 0; i < fList.length; i++) {
      NetLog.d(TAG, "getSmbFileList2 fList[i]:" + fList[i].getName());
      /* filter the format 'D$/' */
      if (fList[i].getName().endsWith("$/"))
        continue;
      mListSmbFile.add(fList[i]);
  ...
  public int getType(String path) throws MalformedURLException, SmbException,
    ...
    if (filename.endsWith(".3gp") || filename.endsWith(".mp4")
      ...
      return TYPE_VIDEO;
        ...
        || filename.endsWith(".amr") || filename.endsWith(".ac4"))
      return TYPE_AUDIO;
  ...
  public LoginInfo getServerInfo(String path) {
    if (path == null || mNamePWDTable.keySet().size() == 0) {
      return null;
    }
    if (mNamePWDTable.containsKey(path) == true && !path.equalsIgnoreCase(mRoot)
        && !path.equalsIgnoreCase(mSecondRoot)) {
      if (localLOGV)
        NetLog.v(TAG, "[getServerInfo]  containsKey path : " + path
            + "  mNamePWDTable.get(path):" + mNamePWDTable.get(path));
      return mNamePWDTable.get(path);
    }
    for (String keyPath : mNamePWDTable.keySet()) {
      if (localLOGV)
        NetLog.v(TAG, "[getServerInfo]  keyPath : " + keyPath);
      if (keyPath.equals(mRoot) || keyPath.equals(mSecondRoot)) {
        continue;
    ...
    if (localLOGV)
      NetLog.v(TAG, "[getServerInfo]  return null : ");
    return null;
  }

  public LoginInfo removePwd(String path) {
    if (path != null && mNamePWDTable.containsKey(path)) {
     return (mNamePWDTable.remove(path));
    }
    return null;
  }
  ...
  private SmbFile getSmbFileIns(String path) throws MalformedURLException,
    ...
    SmbFile smbFile;
//    NetLog.v(taglogin, "[getSmbFileIns] call: " + Log.getStackTraceString(new Throwable()));
    if (localLOGV)
      NetLog.v(TAG, "[getSmbFileIns] new SMB file : " + path);
    ...
    /*
     * if(checkFirstLevel(path)==true && getDomainName(path)!=null) { //smbFile = new SmbFile(path,
     * new NtlmPasswordAuthentication(getDomainName(path), "", "")); //NetLog.d(taglogin,
     * "[getSmbFileIns][Domain]: "+ getDomainName(path)
     * +", new SmbFile(path, new NtlmPasswordAuthentication(getDomainName(path), null, null)");
     * smbFile = new SmbFile(path, mUserAuth); NetLog.d(taglogin,
     * "[getSmbFileIns]: 1--new SmbFile(path, mUserAuth)"); if(mUserAuth!=null) NetLog.d(taglogin,
     * "[getSmbFileIns]: 1--domain: "
     * +mUserAuth.getDomain()+", name: "+mUserAuth.getName()+", pwd: "+mUserAuth.getPassword());
     * return smbFile; }
     */
    ...
    if (mUserAuth != null) {
      ...
      if (localLOGV)
        NetLog.d(TAG,
            "[getSmbFileIns]: new SmbFile(path, NtlmPasswordAuthentication.ANONYMOUS)");
    }
    return smbFile;
  }
