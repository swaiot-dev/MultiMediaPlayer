...
public class MusicPlayActivity extends SkyMediaPlayActivity implements GKView.LoadListener {
    ...
    private static final int MSG_DEMISS = 0x00103;
...
//		clearLrc();
                ...
                case AUTO_HIDE_PROGRESSBAR:
                    ...
                    break;
                case MSG_DEMISS:
//                    Log.d(TAG, "handleMessage: mIsDemiss " + onKeyDown);
                    if (MmpApp.onKeyDown) {
                        hideInfoView();
                    } else {
                        MmpApp.onKeyDown = true;
                        myHandler.sendEmptyMessageDelayed(MSG_DEMISS, 3000);
                    }
                    break;
                default:
                    break;
...
//                mMediaControlView.showPlayStatusLayout();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            //                break;
            //end by yx for 71967
            case KeyMap.KEYCODE_DPAD_CENTER: {                if (isValid()) {
...
//            }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if ((keyCode == KeyMap.KEYCODE_DPAD_LEFT || keyCode == KeyMap.KEYCODE_DPAD_RIGHT
                || keyCode == KeyMap.KEYCODE_MTKIR_REWIND || keyCode == KeyMap.KEYCODE_MTKIR_FASTFORWARD) && !isLongPressLRKey) {
            isLongPressLRKey = false;
...
//      }
    ...
    private boolean seek(int keyCode, KeyEvent event) {
        ...
        if (myHandler.hasMessages(PROGRESS_CHANGED) & key == 3) {
            myHandler.removeMessages(PROGRESS_CHANGED);
        }
//    if (progress < 0) {
//      return true;
...
//        loadCoverInfo();
    ...
    public void showOrHideInfoView() {
        if (null == mInfoDialog) {
            showInfoView();
//            RecHandler.post(RecTimeRunable);
...
//            RecHandler.post(RecTimeRunable);
    ...
    public void showInfoView() {
        ...
        Log.d(TAG, "showInfoView: ");
        myHandler.sendEmptyMessageDelayed(MSG_DEMISS, 5000);
    }
