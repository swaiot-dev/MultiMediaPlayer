package com.mediatek.wwtv.mediaplayer.mmp.carousel;
...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
	...
	public Carousel(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray texts = getResources().obtainTypedArray(R.array.mmp_main_text);
		...
		adapter.SetImages(images, texts, mUseReflection);
...
//        }
	...
	protected boolean getChildStaticTransformation(View child, Transformation transformation) {
		...
		mCamera.translate(((CarouselImageView)child).getMyX(), ((CarouselImageView)child).getMyY(),
							((CarouselImageView)child).getMyZ() - 80);
	...
	void layout(int delta, boolean animate){
        ...
        if (mNextSelectedPosition >= 0) {
        	...
        	MtkLog.d(TAG, "mNextSelectedPosition = " + mNextSelectedPosition);
	...
	private class FlingRotateRunnable implements Runnable {
        ...
        public void startUsingVelocity(float initialVelocity) {
            if (initialVelocity > 0.0f || initialVelocity < 0.0f){
		...
		public boolean isRotatorFinish(){
			return mRotator.isFinished();
		}
	...
	private class ImageAdapter extends BaseAdapter {
		...
		public void SetImages(TypedArray array ,TypedArray texts){
			SetImages(array, texts,true);
		}

		public void SetImages(TypedArray array, TypedArray texts,boolean reflected){
			...
			for(int i = 0; i< array.length(); i++)
			{
				...
				if(reflected){
					...
					MtkLog.d(TAG, "2222,width = " +  width + " height = " + height);
				...
				//	Log.d(TAG, " " + texts.getString(i).length() + " " + width);
					canvas.drawText(texts.getString(i), width / 2 - 15 , height - 4, mPaint);
...
//				imageView.setBackgroundDrawable(getResources().getDrawable(R.drawable.mmp_gridbg));
...
//
    ...
    private void Calculate3DPosition(CarouselImageView child, int diameter, float angleOffset){
    	...
    	float x = -(float)(diameter/1.5*Math.sin(angleOffset));
    	float z = diameter/3 * (1.0f - (float)Math.cos(angleOffset));
	...
	void scrollToChild(int i){
		if(!mFlingRunnable.isRotatorFinish()){
			return ;
		}
...
//        Log.d(TAG, "setUpChild , w = " + w + " h = " + h + " childLeft = " + childLeft + " childTop = " + childTop );
    ...
    public void setOnKeyListener(OnKeyListener onKeyListener){
    	for(int i = 0 ; i < mItemCount ; i++){
    		adapter.getView(i, null, null).setOnKeyListener(onKeyListener);
    	}
    }
